...
fun Login(onSuccess: (email: String) -> Unit, viewModel: LoginViewModel = hiltViewModel()) {
    ...
    Column(
        ...
        EmailInput(
            ...
            value = viewModel.email, onValueChange = viewModel::updateEmail
        )
        PasswordInput(
            modifier = Modifier
                .fillMaxWidth()
                .padding(bottom = MaterialTheme.appDimens.mediumContentPadding),
            value = viewModel.password, onValueChange = viewModel::updatePassword
        )
        PrimaryButton(
            modifier = Modifier.fillMaxWidth(), text = stringResource(id = R.string.login),
...
fun EmailInput(modifier: Modifier, value: String, onValueChange: (String) -> Unit) {
    SingleLineTextInput(
        modifier = modifier, value = value, hint = stringResource(id = R.string.email), onValueChange = onValueChange,
